...
- 文本先经过本地敏感词拦截，再同步调用微信 `msgSecCheck`；群聊消息只有检测通过后才入库，同时保留后台复核、隐藏和审计能力。
- 产品定位统一为“滑雪行程组局工具”。
...
- 后端使用微信 `msgSecCheck` 审核所有可发布文本，使用 `mediaCheckAsync` 审核用户头像和行程图片；阿里云仅用于短信验证码，不再依赖阿里云内容安全。
- 文本检测范围：滑雪局标题、雪场与集合信息、滑雪局备注、用户昵称、用户简介、城市与标签、申请留言、群聊消息、评价内容、举报内容。
- 文本风险时统一返回 `40010` 和“你发布的内容含有违规信息，请修改后重试”；微信接口不可用时拒绝写入。图片审核期间不可公开，安全回调通过后才可使用，风险图片直接拒绝。
...
1. 服务端设置 `CONTENT_SECURITY_ENABLED=true`、`CONTENT_SECURITY_PROVIDER=wechat`，并配置与当前小程序一致的 `WECHAT_APP_ID`、`WECHAT_APP_SECRET`。
2. 生成至少 24 字符的随机 `WECHAT_MEDIA_CALLBACK_TOKEN`，不要提交到仓库。
3. 在微信公众平台配置消息推送地址为 `https://www.xueju.xyz/api/callbacks/wechat/media`，Token 与上一步一致；消息加密方式选择“明文模式”。回调数据格式可选 JSON 或 XML，后端均支持。
4. 保存配置时，微信发送的 GET 校验请求会由同一路由完成签名校验并原样返回 `echostr`。
5. 上线后分别测试昵称修改、行程发布、申请留言、群聊、评价、举报、头像上传和行程图片上传；确认违规文本返回 `40010`，图片在微信回调通过前保持 `pending` 且不能公开。
...
媒体上传后先标记为 `pending` 并调用微信 `mediaCheckAsync`。微信签名回调为 `pass` 后才标记为 `approved`；`risky` 标记为 `rejected`，`review` 或未知结果继续保留为待人工审核。只有 `approved` 文件可以通过公开地址访问。
