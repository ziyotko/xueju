...
func New(cfg config.Config, db *sql.DB) *gin.Engine {
	...
	auth.POST("/uploads/avatar", middleware.RateLimitByUser(20, time.Hour), appHandler.UploadAvatar)
	auth.POST("/uploads/event-image", middleware.RateLimitByUser(20, time.Hour), appHandler.UploadEventImage)
	...
	auth.POST("/events/:id/apply", middleware.RateLimitByUser(20, time.Hour), appHandler.ApplyEvent)
	...
	auth.POST("/events/:id/messages", middleware.RateLimitByUser(60, time.Minute), appHandler.SendMessage)
	auth.POST("/events/:id/messages/read", appHandler.MarkChatRead)
	auth.POST("/reviews", appHandler.CreateReview)
	auth.POST("/reports", middleware.RateLimitByUser(10, time.Hour), appHandler.CreateReport)
